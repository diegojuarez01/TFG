...
$query = "select * from partidos_individuales order by id_partido desc limit 1";
...
if($resultado==0){
    ...
    $query = "insert partidos_individuales(jugador1,jugador2,ganador,fecha_partido) values ('$jugador1','$usuario','Partido pendiente','$fecha')";
    $resultado = $mysqli->query($query);
    
    $query = "update busqueda_partido_1v1_lol set jugador2='$usuario',estado='encontrado' where num_busqueda_partido_1v1_lol = '$id'";
    $resultado = $mysqli->query($query);
    ...
    echo "Error al acceder a la base de datos de usuario.<br>";	
     ...
     setTimeout(function() {
            swal({
                ...
                window.location = 'detallespartidoavanzados.php?idpartido='+$idpartido;
            });
        }, 200);
    </script>";		
        }
    ...
    $query = "insert busqueda_partido_1v1_lol(jugador1,estado) values ('$usuario','buscando')";
    $resultado = $mysqli->query($query);	
    if($resultado==0){
    echo "Error al acceder a la base de datos de usuario.<br>";	
    }else{ 
...
$(document).ready(function() {
    function Contador(){
        ...
        $.ajax({
            type: "GET", 
            url: "completarbusquedapartido.php?id_busqueda="+id_busqueda,
               ...
               $('#resultado').text(data);   
                   ...
                   if(count == 60){
                       ...
                       if(resultado == ''){
                         ...
                         location.href='detallespartidoavanzados.php?idpartido='+id_partido;
                     }
    }
    setInterval(Contador, 1000);
});
 function DetenerCuenta(){
       location='cancelarbusquedapartido.php?id_busqueda='+id_busqueda;           
    }  
</script>
