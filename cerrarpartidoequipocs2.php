...
<?php
   ...
   //Si todo va bien hacemos un update a la tabla con el idpartido recogido, estableciendo el ganador.	
    $query = "update partidos_equipo_cs set ganador='$ganador',kills_local='$killslocal',kills_visitante='$killsvisitante' where id_partido_equipo_cs=$idpartido";
    $resultado = $mysqli->query($query);
    if($resultado==0){
    echo " <h4>Error al modificar el partido en la Base de Datos.</h4><br>";
    }else{
     //Sacamos las puntuaciones de los usuarios enfrentados.   
   $query = "select * from equipo where nombre='$local'";
    $resultado = $mysqli->query($query);
    if($resultado==0){
    echo "Este usuario no esta registrado.<br>";
    }else{
    while($fila=$resultado->fetch_assoc()){
    $puntuacionlocal=$fila["puntuacioncs"];
    }
    }
    //Sacamos las puntuaciones de los usuarios enfrentados.   
    $query = "select * from equipo where nombre='$visitante'";
    $resultado = $mysqli->query($query);
    if($resultado==0){
    echo "Este usuario no esta registrado.<br>";
    }else{
    while($fila=$resultado->fetch_assoc()){
    $puntuacionvisitante=$fila["puntuacioncs"];
    }
    }
...
//Debemos de quitar y dar los puntos a ganador y perdedor
    ...
    $query = "update equipo set puntuacioncs='$puntuacionlocal' where nombre='$local'";
    $resultado = $mysqli->query($query);
        if($resultado==0){
        echo " <h4>Error al modificar el usuario en la Base de Datos.</h4><br>";
        }else{
    $puntuacionvisitante= $puntuacionvisitante - $handicapnegativo;
    $query = "update equipo set puntuacioncs='$puntuacionvisitante' where nombre='$visitante'"; 
    $resultado = $mysqli->query($query);
    $query = "update equipo set partidos_equipo_ganadoscs= partidos_equipo_ganadoscs + 1 where nombre='$local'";
    $resultado = $mysqli->query($query);
    $query = "update equipo set partidos_equipo_perdidoscs= partidos_equipo_perdidoscs + 1 where nombre='$visitante'";
    $resultado = $mysqli->query($query);
    ...
    $query = "update equipo set puntuacioncs='$puntuacionlocal' where nombre='$local'";
    $resultado = $mysqli->query($query);
      $query = "update equipo set partidos_equipo_ganadoscs= partidos_equipo_ganadoscs + 1 where nombre='$visitante'";
    $resultado = $mysqli->query($query);
    $query = "update equipo set partidos_equipo_perdidoscs= partidos_equipo_perdidoscs + 1 where nombre='$local'";
    $resultado = $mysqli->query($query);
       echo "<script type='text/javascript'>
       setTimeout(function() {
              swal({
