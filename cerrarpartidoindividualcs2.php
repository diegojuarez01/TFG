...
$query = "update partidos_individuales_cs set ganador='$ganador',kills_local='$killslocal',kills_visitante='$killsvisitante' where id_partido_cs=$idpartido";
...
//Sacamos las puntuaciones de los usuarios enfrentados.   
    ...
    $query = "update usuarios set puntuacioncs='$puntuacionlocal' where usuario='$local'";
    $resultado = $mysqli->query($query);
        if($resultado==0){
        echo " <h4>Error al modificar el usuario en la Base de Datos.</h4><br>";
        }else{
       $puntuacionvisitante= $puntuacionvisitante - $handicapnegativo;
    $query = "update usuarios set puntuacioncs='$puntuacionvisitante' where usuario='$visitante'"; 
    $resultado = $mysqli->query($query);
        $query = "update usuarios set partidos_usuario_ganadoscs= partidos_usuario_ganadoscs + 1 where usuario='$local'";
    $resultado = $mysqli->query($query);
    $query = "update usuarios set partidos_usuario_perdidoscs= partidos_usuario_perdidoscs + 1 where usuario='$visitante'";
    $resultado = $mysqli->query($query);
    ...
    $query = "update usuarios set puntuacioncs='$puntuacionlocal' where usuario='$local'";
    $resultado = $mysqli->query($query);
    $query = "update usuarios set partidos_usuario_ganadoscs= partidos_usuario_ganadoscs + 1 where usuario='$visitante'";
    $resultado = $mysqli->query($query);
    $query = "update usuarios set partidos_usuario_perdidoscs= partidos_usuario_perdidoscs + 1 where usuario='$local'";
    $resultado = $mysqli->query($query);
       echo "<script type='text/javascript'>
       setTimeout(function() {
              swal({
