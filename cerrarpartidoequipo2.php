...
<?php
   ...
   //Si todo va bien hacemos un update a la tabla con el idpartido recogido, estableciendo el ganador.	
    $query = "update partidos_equipo set ganador='$ganador',kills_local='$killslocal',kills_visitante='$killsvisitante' where id_partido_equipo=$idpartido";
...
//Debemos de quitar y dar los puntos a ganador y perdedor
    ...
    $query = "update equipo set puntuacion='$puntuacionlocal' where nombre='$local'";
    $resultado = $mysqli->query($query);
        if($resultado==0){
        echo " <h4>Error al modificar el usuario en la Base de Datos.</h4><br>";
        }else{
    $puntuacionvisitante= $puntuacionvisitante - $handicapnegativo;
    $query = "update equipo set puntuacion='$puntuacionvisitante' where nombre='$visitante'"; 
    $resultado = $mysqli->query($query);
    $query = "update equipo set partidos_equipo_ganados= partidos_equipo_ganados + 1 where nombre='$local'";
    $resultado = $mysqli->query($query);
    $query = "update equipo set partidos_equipo_perdidos= partidos_equipo_perdidos + 1 where nombre='$visitante'";
    $resultado = $mysqli->query($query);
    ...
    $query = "update equipo set puntuacion='$puntuacionlocal' where nombre='$local'";
    $resultado = $mysqli->query($query);
     $query = "update equipo set partidos_equipo_ganados= partidos_equipo_ganados + 1 where nombre='$visitante'";
    $resultado = $mysqli->query($query);
    $query = "update equipo set partidos_equipo_perdidos= partidos_equipo_perdidos + 1 where nombre='$local'";
    $resultado = $mysqli->query($query);
       echo "<script type='text/javascript'>
       setTimeout(function() {
              swal({
